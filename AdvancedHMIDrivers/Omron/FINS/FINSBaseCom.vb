...
Namespace Omron
    Public MustInherit Class FINSBaseCom
        Inherits MfgControl.AdvancedHMI.Drivers.Omron.OmronBase
...
#Region "Subscription"
        ...
        ''**************************************************************
        'Private Shared Function SortPolledAddresses(ByVal A1 As MfgControl.AdvancedHMI.Drivers.Omron.SubscriptionInfo, ByVal A2 As MfgControl.AdvancedHMI.Drivers.Omron.SubscriptionInfo) As Integer
        ...
        '                    Using fw As New System.IO.StreamWriter("." & System.IO.Path.DirectorySeparatorChar & "DriverErrorLog.log", True)
        ...
        '                    'With {
        '                    '    .Tag = SubscriptionList(i).ID
        '                    '}

        '                    Try
        '                        GroupedSubscriptionReads.TryAdd(GroupedSubscriptionReads.Count, ReadAddress)
        '                    Catch ex1 As Exception
        '                        Dim fw As New System.IO.StreamWriter("." & System.IO.Path.DirectorySeparatorChar & "DriverErrorLog.log", True)
...
#Region "Public Methods"
        ...
        'Private BeginReadLock As New Object
        Public Overrides Function BeginRead(ByVal address As MfgControl.AdvancedHMI.Drivers.Omron.OmronPlcAddress) As Integer
            ...
            'Console.WriteLine("Request created-" & CurrentTNS)
            'If Requests(CurrentTNS) Is Nothing Then
            '    Console.WriteLine("Request is nothing")
            'Else
            '    Console.WriteLine("Request is NOT nothing")
            ...
            '* V3.99y
            Dim ElementCount As Integer '= Requests(CurrentTNS).NumberOfElements
            'If Requests(CurrentTNS).IsLong Or Requests(CurrentTNS).IsFloat Then
            '    ElementCount = Requests(CurrentTNS).NumberOfElements * 2
            'End If
            ElementCount = MfgControl.AdvancedHMI.Drivers.Modbus.ModbusUtilities.GetNumberOfRequiredRegisters(Requests(CurrentTNS))

            Dim data() As Byte = {Requests(CurrentTNS).MemoryAreaCode, CByte((Requests(CurrentTNS).ElementNumber >> 8) And 255), CByte(Requests(CurrentTNS).ElementNumber And 255),
                                      CByte(BitNumberByte), CByte((ElementCount >> 8) And 255), CByte(ElementCount And 255)}
            FINSPacket = New MfgControl.AdvancedHMI.Drivers.Omron.FINSFrame(Header, 1, 1, data, MyObjectID, CurrentTNS)
        ...
        Public Function ReadClock() As String()
            ...
            Dim a As New MfgControl.AdvancedHMI.Drivers.Omron.OmronPlcAddress With {
                .NumberOfElements = 7,
                .BitsPerElement = 8,
                .Address = "CLOCK"
            }
        ...
        Public Overrides Function BeginWrite(ByVal address As MfgControl.AdvancedHMI.Drivers.Omron.OmronPlcAddress, ByVal dataToWrite() As String) As Integer
            ...
            Dim dataPacket As New List(Of Byte) From {
                MemoryAreaCode,
                CByte((address.ElementNumber >> 8) And 255),
                CByte((address.ElementNumber) And 255)
            }
            ...
            Dim ElementCount As Integer = address.NumberOfElements
            If address.IsLong Or address.IsFloat Then
                ElementCount *= 2
            End If

            dataPacket.Add(CByte((ElementCount >> 8) And 255))
            dataPacket.Add(CByte((ElementCount) And 255))

            If address.BitsPerElement = 16 Then
                Dim x(1) As Byte
                For i As Integer = 0 To dataToWrite.Length - 1
                    dataPacket.AddRange(address.ValueToBytes(dataToWrite(i)))
                Next
            Else
                '* Bit level
                For i As Integer = 0 To dataToWrite.Length - 1
            ...
            '* MR=1, SR=2
            Dim FINSPacket As New MfgControl.AdvancedHMI.Drivers.Omron.FINSFrame(Header, 1, 2, dataPacket.ToArray, MyObjectID, CurrentTNS)
        ...
        Public Function WriteClock(ByVal dataToWrite() As String) As String()
            ...
            '* Mark as a write and Save
            Dim address As New MfgControl.AdvancedHMI.Drivers.Omron.OmronPlcAddress With {
                .IsWrite = True,
                .BitsPerElement = 8,
                .NumberOfElements = dataToWrite.Length
            }
...
#Region "Private Methods"
        ...
        Protected Sub DataLinkLayerDataReceived(ByVal sender As Object, ByVal e As MfgControl.AdvancedHMI.Drivers.Common.PlcComEventArgs)
            ...
            If e.RawData Is Nothing OrElse e.RawData.Length < 12 Then
                waitHandle(e.TransactionNumber).Set()
                Exit Sub
            End If
            ...
            e.ResponseFrame = Fins

            '********************************* HACK **************************************
            '* For some reason an incomplete packet is return even though WireShark Shows
            '* complete. If this happens, then ignore
            '* !!! SEEMS TO BE A RESPONSE FROM A WRITE
            '******************************************************************************
            'If ((Fins.CommandData.Count - 2) < Requests(Fins.Header.ServiceId).NumberOfElements And Requests(Fins.Header.ServiceId).MemoryAreaCode < &H80) Or
            '    ((Fins.CommandData.Count * 2 - 2) < Requests(Fins.Header.ServiceId).NumberOfElements And Requests(Fins.Header.ServiceId).MemoryAreaCode >= &H80) Then
            '    waitHandle(Fins.Header.ServiceId).Set()
            '    'Console.WriteLine(Fins.TransactionNumber & "," & e.TransactionNumber & "," & Fins.Header.ServiceId)
            '    'Console.WriteLine("RawLength=" & e.RawData.Length & ",FinsCDLength=" & Fins.CommandData.Count)
            '    'Console.WriteLine("RequestLength=" & Requests(e.TransactionNumber).NumberOfElements)
            '    'Console.WriteLine("Skipped packet. IDs=" & e.OwnerObjectID & "," & MyObjectID)
            '    Exit Sub
            'End If


            Requests(Fins.Header.ServiceId).Response = e
            'Dim HostFrame As New MfgControl.AdvancedHMI.Drivers.Omron.HostLinkFrame(e.RawData)

            '* 9-MAR-12 V1.15
            If Requests(Fins.Header.ServiceId) Is Nothing OrElse Requests(Fins.Header.ServiceId).Response Is Nothing _
                OrElse (Fins.Header.ServiceId < 0 Or Fins.Header.ServiceId > (Requests.Length - 1)) Then
                Exit Sub
            End If

            e.PlcAddress = Requests(Fins.Header.ServiceId).Address

            '* V3.99y -  Ignore bits 6 and 7 which are CPU status bits
            '* Ref W342-E1-14 page 155
            If (Fins.EndCode And &HFF3F) = 0 Then
                'If Fins.EndCode = 0 Then
                If Not Requests(Fins.Header.ServiceId).IsWrite Then
                    '* Extract the data - First 2 bytes is the End Code (status)
                    'Dim values() As String = ExtractData(Fins.CommandData, 2, Requests(Fins.Header.ServiceId))
                    Dim values() As String = MfgControl.AdvancedHMI.Drivers.Modbus.ModbusUtilities.ExtractData(DirectCast(Requests(Fins.Header.ServiceId).Response.ResponseFrame,
                                                                         MfgControl.AdvancedHMI.Drivers.Omron.OmronBaseFrame).CommandData, 0, Requests(Fins.Header.ServiceId), True)
                    For i As Integer = 0 To values.Length - 1
                        e.Values.Add(values(i))
                    Next

                    '* Verify that enough elements were returned before continuing V1.11 6-MAR-12
                    If e.Values.Count >= Requests(Fins.Header.ServiceId).NumberOfElements Then
                    ...
                    End If
                End If
                'Requests(Fins.Header.ServiceId).Responded = True
                waitHandle(Fins.Header.ServiceId).Set()
            Else
                ...
                OnComError(e)
                Requests(Fins.Header.ServiceId).ErrorReturned = True
            End If
...
#End Region
        ...
        '            values(i) = address.FINSBytesToValue(RawData, startByte + i * NumberOfBytes)
        '        Else
