...
Namespace Omron
    Public MustInherit Class HostLinkBaseCom
        ...
        Friend MustOverride Function SendData(ByVal HostLinkF As MfgControl.AdvancedHMI.Drivers.Omron.HostLinkFrame) As Boolean
...
#Region "Public Methods"
        ...
        Public Overrides Function BeginRead(ByVal address As MfgControl.AdvancedHMI.Drivers.Omron.OmronPlcAddress) As Integer
            ...
            Try
                ...
                MfgControl.AdvancedHMI.Drivers.Common.CalculationsAndConversions.ValueToBcdAscii(ElementCount).CopyTo(b, 4)
            ...
            Try
                If SendData(HostLinkPacket) Then
        ...
        Public Overrides Function BeginWrite(ByVal address As MfgControl.AdvancedHMI.Drivers.Omron.OmronPlcAddress, ByVal dataToWrite() As String) As Integer
            ...
            dataPacket.Add(MemoryAreaCode)
            dataPacket.Add(Convert.ToByte((address.ElementNumber >> 8) And 255))
            dataPacket.Add(Convert.ToByte((address.ElementNumber) And 255))
            dataPacket.Add(Convert.ToByte(address.BitNumber And 255))
            dataPacket.Add(Convert.ToByte((address.NumberOfElements >> 8) And 255))
            dataPacket.Add(Convert.ToByte((address.NumberOfElements) And 255))
            ...
            If SendData(HostLinkPacket) Then
                Return HostLinkPacket.TransactionNumber
            Else
                Throw New MfgControl.AdvancedHMI.Drivers.Common.PLCDriverException("Send Buffer is full, may have lost communication.")
            End If
        End Function
        ...
        Public Function ReadProgram() As Byte()
            ...
            Try
                If Not SendData(HostLinkPacket) Then
                    '* Buffer is full, so release
                    Throw New MfgControl.AdvancedHMI.Drivers.Common.PLCDriverException("Send Buffer is full, may have lost communication.")
                End If
            Catch ex1 As Exception
            ...
            Dim TNSByte As Integer = Convert.ToInt32(CurrentTNS And 255)
            If waitHandle(TNSByte).WaitOne(10000) Then  'WaitForResponse(CUShort(index), 10000) = 0 Then
                If Requests(TNSByte).Response IsNot Nothing Then
                    '* Remove the @00RP.. from the rawdata and the FCS, CR, and *
                    Dim tmp(Requests(TNSByte).Response.RawData.Length - 11) As Byte
                    For i As Integer = 0 To tmp.Length - 1
                        tmp(i) = Requests(TNSByte).Response.RawData(i + 7)
                    Next
                    Return tmp
                Else
                ...
                End If
            Else
                If Requests(TNSByte).ErrorReturned Then
                    Throw New MfgControl.AdvancedHMI.Drivers.Common.PLCDriverException("Error Returned " & Requests(TNSByte).Response.ErrorId)
                Else
                    Throw New MfgControl.AdvancedHMI.Drivers.Common.PLCDriverException("No Reponse from PLC. Ensure baud rate is correct.")
                End If
            End If
        ...
        Public Function ChangeStatus(ByVal Mode As Byte) As Integer
            ...
            '****************************************
            Dim TNSByte As Integer = Convert.ToInt32(CurrentTNS)
            If waitHandle(TNSByte).WaitOne(10000) Then ' WaitForResponse(CUShort(index), 10000) = 0 Then
                If Requests(TNSByte).Response IsNot Nothing Then
                    Return 0
                Else
...
#Region "Private Methods"
        ...
        Protected Sub DataLinkLayerDataReceived(ByVal sender As Object, ByVal e As MfgControl.AdvancedHMI.Drivers.Common.PlcComEventArgs)
            ...
            Requests(TNSByte).Response = e
            Dim HostFrame As New MfgControl.AdvancedHMI.Drivers.Omron.HostLinkFrame(e.RawData)
            Requests(TNSByte).Response.ResponseFrame = HostFrame

            If HostFrame.EndCode <> 0 Then
                OnComError(New MfgControl.AdvancedHMI.Drivers.Common.PlcComEventArgs(HostFrame.EndCode, "Error Returned From PLC=" & DecodeHostLinkEndCode(HostFrame.EndCode)))
                'Requests(TNSByte).Responded = True
                waitHandle(TNSByte).Set()
                Exit Sub
            End If

            '* 9-MAR-12 V1.15
            If Requests(TNSByte) Is Nothing OrElse Requests(TNSByte).Response Is Nothing _
            ...
            If Not Requests(TNSByte).IsWrite Then
                '* Extract the data
                'Dim values() As String = ExtractData(HostFrame.CommandData, 0, Requests(TNSLowerbyte))
                Dim values() As String = MfgControl.AdvancedHMI.Drivers.Modbus.ModbusUtilities.ExtractData(DirectCast(Requests(TNSByte).Response.ResponseFrame,
                                                                         MfgControl.AdvancedHMI.Drivers.Omron.OmronBaseFrame).CommandData, 0, Requests(TNSByte), True)
